...
**The bug:** In setup, if you only call `bob1.waitForMessage()` to drain the FRIEND_REQUEST, bob2's WebSocket queue still has it. When test 2 calls `bob2.waitForMessage()` expecting APP_ENTRY, it gets the stale FRIEND_REQUEST instead.
