...
**How to apply:** Any scenario test that calls `connect()`, `waitForMessage()`, or does real server
I/O: use `= runBlocking { }`. Pure local store or policy tests can use `= runTest { }`.
